...
<script type="text/javascript">
	$(function() {
		...
		$('#form')
				.form(
						{
							url : '${pageContext.request.contextPath}/lotteryconf/awardconf/add.json',
							onSubmit : function() {
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title=""
		...
		map.put("startTimeFloat", startTimeFloat);
		map.put("endTimeFloat", endTimeFloat);
		...
		map.put("details", details); -->
				...
				<tr>
					...
					<td>${internationalConfig.每隔} <c:if test="${currentLanguage=='en'}"><c:out value=" "/></c:if><input type="text" name="minutes" id="easyui-numberbox"
						...
						class="easyui-validatebox" data-options="required:false" /><c:if test="${currentLanguage=='en'}"><c:out value=" "/></c:if>${internationalConfig.分钟}，${internationalConfig.发放}<c:if test="${currentLanguage=='en'}"><c:out value=" "/></c:if><input
						type="text" name="total" id="easyui-numberbox" style="width: 35px"
						data-options="min:0,max:10000,precision:0"
						class="easyui-validatebox" data-options="required:false" /><c:if test="${currentLanguage=='en'}"><c:out value=" "/></c:if>${internationalConfig.个奖品}
					</td>
				</tr>
