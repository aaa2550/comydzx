...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'center',border:false" title="">
        <form id="form" method="post" action="">
            <input type="hidden" name="otherId" value="${props.otherId}">
            <table style="width: 100%" class="table table-form">
                ...
                <tr>
                    <th>第三方动效：</th>
                    <td>
                        <input type="hidden" name="otherFile" value="${props.otherFile}">
                        <c:set value="${ fn:split(props.otherFile, ',') }" var="otherFiles" />
                        <c:forEach items="${ otherFiles }" var="otherFile">
                            <input name="other_file" value="${otherFile}" class="span2, easyui-validatebox" readonly><a href="javascript:window.open('otherFile')">下载</a></br>
                        </c:forEach>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <%@ include file="/WEB-INF/views/inc/props_unit.inc" %>
                    </td>
                </tr>
                <tr>
                    <th>属性配置：</th>
                    <td>
                        <input type="checkbox" value="1" name="attributeConfigs">推荐&nbsp;&nbsp;
                        <input type="checkbox" value="2"  name="attributeConfigs">可连击&nbsp;&nbsp;
                        <input type="checkbox" value="4" name="attributeConfigs">可通知所有人&nbsp;&nbsp;
                    </td>
                </tr>
                <%--<tr>
                    ...
                    <td>
                        <select name="flag" class="span2">
                            ...
                            <option value="2" <c:if test="${2 eq props.flag}">selected</c:if>>已处理</option>
                        </select>
                    </td>
                </tr>--%>
                <tr>
...
*/
    function init() {
        ...
        //初始化属性
        <c:if test="${props.id > 0}">
            $("input:checkbox[name='property_ids']").each(function () {
                <c:if test="${not empty props.propertyIds}">
                    var k = 0;
                    <c:set value="${ fn:split(props.propertyIds, ',') }" var="propertyIds" />
                    <c:forEach items="${ propertyIds }" var="propertyId">
                ...
                <c:if test="${empty props.propertyIds}">
                    $(this).removeAttr('checked');
                </c:if>
            });
        </c:if>
    ...
    function validateCodeName(){
        ...
        $.ajax({
            type: "POST",
            url: "/props/libraryOther/check",
            data: {"id":id, "codeName":codeName},
            dataType: 'json',
    ...
    function preparePropertyIds() {
        var propertyIds = '';
        var inputs = $("input[name='property_ids']");
        inputs.each(function(i) {
            if ($(this).is(':checked')) {
                if (i > 0) {
                    propertyIds += ',';
                }
                propertyIds += $(this).val();
            }
        })
