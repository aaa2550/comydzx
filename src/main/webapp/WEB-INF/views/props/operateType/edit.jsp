...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'center',border:false" title="">
        <form id="form" method="post" action="/props/operateType/save">
            <input type="hidden" name="id" value="${propsOperateType.id}">
            <table style="width: 100%" class="table table-form">
                <tr>
                    <th><b style="color: red">*</b>分类名称：</th>
                    <td>
                        <input name="name" value="${propsOperateType.name}" class="span2, easyui-validatebox" data-options="required:true">&nbsp;&nbsp;
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>排序：</th>
                    <td><input name="no" value="${propsOperateType.no}" class="span2, easyui-validatebox">数值越小的排在前面</td>
                </tr>
            </table>
        </form>
    </div>
...
<script type="text/javascript">
    ...
    function init() {
        ...
        <c:if test="${propsOperateType.businessId > 0}">
        businessId.val('${propsOperateType.businessId}');
        </c:if>
        businessId.trigger("change");

        //初始化渠道
        var channelId = $("select[name=channelId]");
        <c:if test="${propsOperateType.channelId > 0}">
        channelId.val('${propsOperateType.channelId}');
        </c:if>
