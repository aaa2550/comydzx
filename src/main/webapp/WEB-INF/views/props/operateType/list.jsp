...
<head>
    ...
    <script type="text/javascript">
        var dataGrid;
        $(function () {
            dataGrid = renderDataGrid('/props/operateType/query');
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                var str = $.formatString("<a href='javascript:void(0);' onclick='editOperateType({0}, \"{1}\")' class='propsOperation'>修改</a>", row.id, '修改分类');
                                str += $.formatString("<a href='javascript:void(0);' onclick='delOperateType({0})' class='propsOperation'>删除</a>", row.id);
                                return str;
                            }
                        }
                    ]
         ...
         * 添加修改分类
         */
        function editOperateType(id, title) {
            parent.$.modalDialog({
                title: title,
                width: 500,
                height: 400,
                href: '${pageContext.request.contextPath}/props/operateType/edit?id='+id,
                buttons: [
                    {
                        text: '保存',
                        handler: function () {
         ...
         * 删除
         */
        function delOperateType(id){
            $.messager.confirm("询问", "确认删除运营分类吗？", function(b){
                if(!b) return;
                $.post('/props/operateType/delete', {id: id},
                        function(json){
                            if(json.code == 0){
                                $.messager.alert("删除运营分类", "删除运营分类成功！");
                                dataGrid.datagrid('load', {});
                            } else {
                                $.messager.alert("删除运营分类", json.msg);
                            }
                        }, "json"
                );
            });
...
<div id="toolbar" style="display: none;">
    ...
    <a onclick="editOperateType(0, '添加分类');" href="javascript:void(0);"
