...
<head>
    ...
    <script type="text/javascript">
        var dataGrid;
        $(function () {
            dataGrid = renderDataGrid('/props/unit/query');
        });
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                if (row.isDefault==1) {
                                    return $.formatString("<a href='javascript:void(0);' onclick='editUnit({0}, \"{1}\")' class='propsOperation'>修改</a>", row.id, '修改单位');
                                } else {
                                    var str = $.formatString("<a href='javascript:void(0);' onclick='setDefault({0})' class='propsOperation'>设为默认</a>", row.id);
                                    str += $.formatString("<a href='javascript:void(0);' onclick='editUnit({0}, \"{1}\")' class='propsOperation'>修改</a>", row.id, '修改单位');
                                    /*str += $.formatString("<a href='javascript:void(0);' onclick='delUnit({0})' class='propsOperation'>删除</a>", row.id);*/
                                    return str;
         ...
         * 添加修改单位
         */
        function editUnit(id, title) {
            parent.$.modalDialog({
                title: title,
                width: 400,
                height: 300,
                href: '${pageContext.request.contextPath}/props/unit/edit?id='+id,
                buttons: [
                    {
                        text: '保存',
                        handler: function () {
        ...
        function setDefault(id){
            $.messager.confirm("询问", "确认设置默认吗？", function(b){
                if(!b) return;
                $.post('/props/unit/setDefault', {id: id},
                        function(json){
                            if(json.code == 0){
         ...
         */
        function delUnit(id){
            $.messager.confirm("询问", "确认删除道具单位吗？", function(b){
                if(!b) return;
                $.post('/props/unit/delete', {id: id},
                        function(json){
                            if(json.code == 0){
                                $.messager.alert("删除道具", "删除道具成功！");
                                dataGrid.datagrid('load', {});
                            } else {
                                $.messager.alert("删除道具", "删除道具失败！");
                            }
                        }, "json"
                );
            });
...
<div id="toolbar" style="display: none;">
    <a onclick="editUnit(0, '添加单位');" href="javascript:void(0);"
       class="easyui-linkbutton" data-options="plain:true,iconCls:'pencil_add'">添加单位</a>
</div>
