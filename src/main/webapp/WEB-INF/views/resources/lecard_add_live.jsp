			...
			<table style="margin-left: 30px; margin-top: 20px">
				...
				<tr>
					<th>${internationalConfig.乐卡类型}</th>
                            ...
                            <td style="width: 100px">
                                <label>
                                     <input type="radio" value="${applyType }" name="applyType"  checked="checked">${applyTypeName }
                                </label>
                            </td>
				</tr>
				...
				<tr id="dhtc" >
					...
					<td><label> <select name="applyPackage"
							...
							<option		data-amount="0"  value="0">${internationalConfig.通用兑换码}</option>
								<c:forEach items="${eplPackage}" var="item">
			<option		value="${item.id}"  data-amount="${item.regular_price }">${item.playName}</option>
