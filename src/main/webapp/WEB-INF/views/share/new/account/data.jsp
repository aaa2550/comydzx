                    ...
                    dataGrid = $('#dataGrid1').datagrid({
                        ...
                        columns: [[{
                            ...
                            formatter: function (value, row, index) {
                                var str = '';
                                if (row.status == 0) {
                                    str = String.format("<a href='javascript:void(0);' onclick='changeStatus(\"" + row.time + "\"," + row.ruleId + ")'>${internationalConfig.结算}</a>|", row.id);
                                }
                                str += String.format("<a target='_blank' href='/account/details/list.do?cid=" + row.cid + "&configType=" + patamConfigType + "&time=" + row.time + "&albumId="  + row.albumId + "'>${internationalConfig.查看明细}</a>", row.id);
                                return str;
                            }
                        }]],
                        toolbar: '#toolbar',
                    ...
                    dataGrid = $('#dataGrid5').datagrid({
                        ...
                        columns: [[{
                            ...
                            formatter: function (value, row, index) {
                                ...
                                str += String.format("<a target='_blank' href='/account/details/list.do?cid=" + row.cid + "&configType=" + patamConfigType + "&time=" + row.time + "&memberType="  + row.memberType + "'>${internationalConfig.查看明细}</a>", row.id);
                                return str;
                            }
                        }]],
                        toolbar: '#toolbar',
