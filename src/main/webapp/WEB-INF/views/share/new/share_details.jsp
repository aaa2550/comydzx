            ...
            dataGrid = $('#dataGrid').datagrid({
                ...
                columns: [[{
                    ...
                    formatter: function (value, row, index) {
                        ...
                        switch (row.status) {
                            case 0:
                                return String.format("<a href='javascript:void(0);' onclick='changeStatus(" + row.month + "," + row.ruleId + ")'>${internationalConfig.结算}</a>", row.id);
                                break;
                            case 1:
                                return String.format("<a target='_blank' href='/share_details/toDetails?albumId=" + row.albumId + "&configType=" + row.configType + "&month=" + row.month + "'>${internationalConfig.查看明细}</a>", row.id);
                                break;
                            case 2:
                                return String.format("<a target='_blank' href='/share_details/toDetails?albumId=" + row.albumId + "&configType=" + row.configType + "&month=" + row.month + "'>${internationalConfig.查看明细}</a>", row.id);
                                break;
