...
<script type="text/javascript">
	...
	function businessValid() {
		...
		if (addType == '1') {
			...
			if (!nameEn) {
				parent.$.messager.alert('${internationalConfig.错误}', "${internationalConfig.必须填写英文名称}", 'error');
				return false;
			}
			$("input[name='name']").val(nameEn);
		}
		...
		<form id="form" method="post">
			...
			<table class="table">
				<tr ${copyright.id > 0 ? "style='display:none'" : ""}>
					<th></th>
					<td><input type="radio" name="addType" value="1" checked="checked" onclick="changeAddType(1)"/>${internationalConfig.绑定CPID}<input type="radio" name="addType" value="2" onclick="changeAddType(2)"/>${internationalConfig.匹配CP名称}</td>
				</tr>
				<tr class="addType1">
