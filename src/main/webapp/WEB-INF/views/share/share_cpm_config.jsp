...
<m:auth uri="/share_cpm_config/delete.json">  $.canDelete = true;</m:auth>
<m:auth uri="/share_cpm_config/edit.json">  $.canEdit = true;</m:auth>
	var dataGrid;
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/share_cpm_config/data_grid.json',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			}, 
			{
				...
				formatter : function(value, row, index) {
					...
					if ($.canDelete) {
						str += $.formatString('<img onclick="deleteFun(\'{0}\');" src="{1}" title="删除"/>', row.id, '${pageContext.request.contextPath}/static/style/images/extjs_icons/pencil_delete.png');
					}
					return str;
				}
			} ] ],
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			height : 500,
			resizable : true,
			href : '${pageContext.request.contextPath}/share_cpm_config/edit.json?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			height : 500,
			resizable : true,
			href : '${pageContext.request.contextPath}/share_cpm_config/add.json',
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除配置？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/share_cpm_config/delete.json', {
					id : id
				}, function(result) {
					if (result.code == 0) {
