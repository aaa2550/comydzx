...
<m:auth uri="/share_play_config/delete.json">  $.canDelete = true;</m:auth>
<m:auth uri="/share_play_config/edit.json">  $.canEdit = true;</m:auth>
	var dataGrid;
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/share_play_config/data_grid.json',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			height : 500,
			resizable : true,
			href : '${pageContext.request.contextPath}/share_play_config/edit.json?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			height : 500,
			resizable : true,
			href : '${pageContext.request.contextPath}/share_play_config/add.json',
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除配置？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/share_play_config/delete.json', {
					id : id
				}, function(result) {
					if (result.code == 0) {
