...
<head>
    ...
    <script type="text/javascript">
        ...
        function exportExcel() {
            ...
            location.href = '${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/excel?' + $('#searchForm').serialize();
        }
        ...
        function loadDataGrid() {
            if (!dataGrid) {
                dataGrid = renderDataGrid('${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/dataGrid');
            } else {
                dataGrid.datagrid('load', $.serializeObject($('#searchForm')));
            }
        }
        ...
        function loadCashierChart() {
            var url = '${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/serial';
            ...
            $.post(url, params, function (rows) {
                ...
                $('#serial').highcharts({
                    ...
                    tooltip: {
                        ...
                        pointFormat:"<p><span>{series.name}&nbsp;:&nbsp;</span><span>{point.y:.2f}</span></p><p>"+((params.terminal==112||params.terminal==113)?"<a href='javascript:;' onclick='viewDetails(\"{point.x}\",1)'>查看详情</a>":"")+"</p>",
                        useHTML: true,
                        valueSuffix: '%'
        ...
        function viewDetails(num,add) {
            ...
            $.ajax({
                type: 'POST',
                cache: false,
                url: '${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/serial',
                data: param,
                dataType: 'json',
        ...
        function loadSuitColumn(createDate) {
            var params = $.serializeObject($('#searchForm'))
            $.getJSON('${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/suitColumn', params, function (result) {
        ...
        function loadSuitSerial(createDate,$this) {
            ...
            $.getJSON('${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/suitSerial', params, function (result) {
                ...
                //根据长度设置步长，防止x轴数据太多显示不全
                var step = Math.round(result.categories.length / 6);
        ...
        function loadCurrSummary(type,that) {
            ...
            $.getJSON('${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/currInfo', $.extend($.serializeObject($('#searchForm')), {type: type}), function (result) {
                //总支付数据
        ...
        function loadFunnelGraph(createDate) {
            ...
            $.getJSON('${pageContext.request.contextPath}/tj/legoHookstat/payFlowNew/funnelAnalyze', params, function (result) {
                var graphTitle = '';
                var graph = '';
                var rowNum = 2;
        ...
        <form id="searchForm">
            <table class="table table-hover table-condensed">
                ...
                <tr>
                    ...
                    <td>
                    	<select name="legoApp" style="width:160px;height:29px">
                            <option value="">全部</option>
                            <c:forEach items="${legoApps}" var="legoApp">
                                <option value="${legoApp.key}" <c:if test="${legoApp.key == 1}">selected="selected"</c:if>>${legoApp.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
...
<div id="toolbar" style="display: none;">
    ...
    <div class="tb">
        ...
        <div class="title2 bb detail_title"> <span class="title_text">套餐选择页转化率:</span></div>
        ...
        <div class="title2 bb detail_title"><span class="title_text">商品订单数：</span></div>
        <div class="clearfix">
            <div id="suitColumn" style="width:100%;height:500px;"></div>
        </div>
        <div class="title2 bb detail_title"><span class="title_text">商品订单转化率趋势：</span></div>
        <div class="clearfix">
