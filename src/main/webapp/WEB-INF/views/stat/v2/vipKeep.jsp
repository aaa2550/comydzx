...
<html>
<head>
    <title>${internationalConfig.会员留存查询}</title>
    ...
    <script type="text/javascript">
        ...
        var dataGrid;
        $(function () {
            dataGrid = renderDataGrid('${pageContext.request.contextPath}/tj/v2order/vipKeep/dataGrid');
        });
            ...
            return $('#dataGrid').datagrid({
                ...
                columns: [
                    [
                        ...
                        field: 'vipKeepCount',
                        title: '${internationalConfig.会员留存数}',
                        width: 100,
                        sortable: true
                    },
                        {
                            field: 'accumulativeExpireCount',
                            title: '${internationalConfig.累计过期用户数}',
                            width: 100,
                            sortable: true
                        },
                        {
                            field: 'dayExpireCount',
                            title: '${internationalConfig.当日过期用户数}',
                            width: 100,
                            sortable: true
                        }
                    ]
        ...
        function loadChart(rows) {
            $('#container').highcharts({
                ...
                title: {
                    text: '${internationalConfig.会员留存趋势}',
                    x: -20 //center
                },
                xAxis: {
                ...
                series: [
                    {
                        type: 'spline',
                        name: '${internationalConfig.会员留存数}',
                        data: $.map(rows, function (element) {
                            return Number(element['vipKeepCount']);
                        }),
                        color: '#3890CD'
                    },
                    {
                        type: 'spline',
                        name: '${internationalConfig.累计过期用户数}',
                        data: $.map(rows, function (element) {
                            return Number(element['accumulativeExpireCount']);
                        }),
                        color: '#22BA66'
                    }
                ]
...
<div id="dataExplain" closed="true" class="dataExplain easyui-dialog">
    <p>会员留存数: 到期时间大于等于当天+1的所有会员数</p>

    <p>累计过期用户数: 到期时间小于当天+1的所有会员数</p>

    <p>当日过期用户数: 到期时间是今天的所有会员数</p>
</div>
<div class="easyui-layout" data-options="fit : true,border : false">
    <div data-options="region:'north',title:'${internationalConfig.查询条件}',border:false"
         style="height: 120px; overflow: auto;">
        <form id="searchForm" action="${pageContext.request.contextPath}/tj/v2order/vipKeep/excel" method="post">
            <table class="table table-hover table-condensed">
                ...
                <tr>
                    ...
                    <td>
                        <input name="dateStart" id="begin" class="easyui-datebox"  data-options="required:true" value="${dateStart}" style="width: 160px; height: 29px"/>
                    </td>

                    <td>
                        <input name="dateEnd" id="end" class="easyui-datebox"  data-options="required:true" value="${dateEnd}" style="width: 160px; height: 29px">
                    </td>
                    <td>
...
<div id="toolbar" style="display: none;">
    ...
    <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'brick_go',plain:true" onclick="exportExcel();">${internationalConfig.导出excel}</a>
    <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'help',plain:true" onclick="dataExplain();">${internationalConfig.字段解释}</a>
