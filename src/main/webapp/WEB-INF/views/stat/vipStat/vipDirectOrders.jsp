...
<head>
    ...
    <script type="text/javascript">
        ...
        var dataGrid;
        $(function () {
            dataGrid = renderDataGrid('${pageContext.request.contextPath}/vipController/vipDirectQueryList');
        });
        ...
        function exportExcel() {
        	...
        	var url = '${pageContext.request.contextPath}/vipController/exportVipDirectExcel?beginDate='+begin + '&endDate=' + end + '&terminal=' + terminal + '&terminal2=' + terminal2 + '&orderpaytype=' + orderpaytype + '&orderpaytype1=' + orderpaytype1;
        	location.href= url ;
        }
	   	        ...
	   	        series: [{
	   	            name: '总用户数',
	   	             data: $.map(rows, function (element) {
							...
							return Number(element['allVip']) ; 
	   	                })
	   	        },
	   	        {
	   	        name: '点播用户数',
	               data: $.map(rows, function (element) {
	            	   return Number(element['videoVip']) ; 
	                  })
	          	   },
	          		{
	      	        name: '移动影视会员数',
	                  data: $.map(rows, function (element) {
	                	  return Number(element['ptVip']) ; 
	                     })
	             	   },
	             	 {
	             	        name: '全屏影视会员数',
	                         data: $.map(rows, function (element) {
	                        	 return Number(element['gjVip']) ; 
	                            })
	                   }]
	   	    });
	   	}
        ...
        <form id="searchForm">
            <table class="table table-hover table-condensed">
                ...
                <tr>
                    ...
                    <td>
                        <input name="endDate" id="end" class="easyui-datebox" validType="TimeCheck" invalidMessage="查询时间范围是60天!!!" data-options="required:true" value="${end}" style="width: 160px; height: 29px">
                    </td>
                    <td>
