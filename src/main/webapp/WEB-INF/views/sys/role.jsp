		...
		treeGrid = $('#treeGrid').treegrid({
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					if (value) {
						return String.format('<span title="{0}">{1}</span>', value, value);
					}
					return '';
				}
			}, {
				...
				formatter : function(value, row, index) {
					var str = '';
					if ($.canEdit) {
						str += String.format('<img onclick="editFun(\'{0}\');" src="{1}" title="${internationalConfig.编辑}"/>', row.id, '/static/style/images/extjs_icons/bug/bug_edit.png');
					}
					str += '&nbsp;';
					if ($.canGrant) {
						str += String.format('<img onclick="grantFun(\'{0}\');" src="{1}" title="${internationalConfig.授权}"/>', row.id, '/static/style/images/extjs_icons/key.png');
					}
					str += '&nbsp;';
					if ($.canDelete) {
						str += String.format('<img onclick="deleteFun(\'{0}\');" src="{1}" title="${internationalConfig.删除}"/>', row.id, '/static/style/images/extjs_icons/bug/bug_delete.png');
					}
					return str;
				}
			} ] ],
