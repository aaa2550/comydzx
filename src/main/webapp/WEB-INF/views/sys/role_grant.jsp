...
<script type="text/javascript">
	...
	$(function() {
		resourceTree = $('#resourceTree').tree({
			url : '${pageContext.request.contextPath}/sys/tree?t=all',
			parentField : 'pid',
			//lines : true,
			checkbox : true,
			...
			onLoadSuccess : function(node, data) {
				var ids = $.stringToList('${role.rids}');
				if (ids.length > 0) {
				...
				$('#roleGrantLayout').layout('panel', 'west').panel('setTitle', String.format('[{0}]${internationalConfig.角色可以访问的资源}', '${role.name}'));
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/role/grant.json',
			onSubmit : function() {
...
<div id="roleGrantLayout" class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'west'" title="${internationalConfig.系统资源}" style="width: 300px; padding: 1px;">
		<div class="well well-small">
			<form id="form" method="post">
				<input name="id" type="hidden" class="span2" value="${role.id}" readonly="readonly">
				<ul id="resourceTree"></ul>
				<input id="rids" name="rids" type="hidden" />
			</form>
		</div>
	</div>
	<div data-options="region:'center'" title="" style="overflow: hidden; padding: 10px;">
		...
		<div class="well well-large">
			<button class="btn btn-success" onclick="checkAll();">${internationalConfig.全选}</button>
			<br /> <br />
			<button class="btn btn-warning" onclick="checkInverse();">${internationalConfig.反选}</button>
			<br /> <br />
			<button class="btn btn-inverse" onclick="uncheckAll();">${internationalConfig.取消}</button>
		</div>
	</div>
</div>