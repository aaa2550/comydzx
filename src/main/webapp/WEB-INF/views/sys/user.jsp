		...
		dataGrid = $('#dataGrid').datagrid({
			...
			columns : [ [  {
				...
				formatter : function(value, row, index) {
					...
					str += '&nbsp;';
					if ($.canGrant) {
						str += $.formatString('<img onclick="grantFun(\'{0}\');" src="{1}" title="${internationalConfig.授权}"/>', row.uid, '/static/style/images/extjs_icons/key.png');
					}
					str += '&nbsp;';
					if ($.canDelete) {
						str += $.formatString('<img onclick="deleteFun(\'{0}\');" src="{1}" title="${internationalConfig.删除}"/>', row.uid, '/static/style/images/extjs_icons/bug/bug_delete.png');
					}
